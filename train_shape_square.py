...
DATA_INPUT = r"F:\study\3_5programtest\8_bolt_change_v3\data_plastic\plastic_data_set_v4_Industrial_camera_6plastic\4_1080p_9PM_arealight1largellittle_lowst_f3.1_striplight4l_4s\org_dataset/"  #用于生成训练集与测试集路径
...
def train(model):
    ...
    train_set = TripleDataset(DATA_INPUT + 'attribute_shape_sixplastic_train.tsv')
    test_set = TripleDataset(DATA_INPUT + 'attribute_shape_sixplastic_test.tsv')  # use triple
    ...
    for epoch in range(epoch_num): #迭代 epoch_num 次。外层迭代，表示训练过程的轮次。
        ...
        if round(acc / (test_set.len*2), 5) >= best_score:
            ...
            name_str = r'./data_plastic\plastic_data_set_v4_Industrial_camera_6plastic\4_1080p_9PM_arealight1largellittle_lowst_f3.1_striplight4l_4s\shape_incloud_colour_data_aug_200transet\checkpoint/model_shape_best_4neg.pkl'.replace('best', str(best_score))
...
def iii():
    #在这个代码中，推理的目标是输出每个属性的推理结果。
    attribute_out_dic={0:'square1_abs',1:'square2_pared',2:'square3_ppwhite',3:'irregularity1_pe',4:'irregularity2_ppblack',5:'irregularity3_pvc',6:'square4_pa',7:'square5_abswhite',8:'irregularity4_pu'} #一个字典，将索引映射到相应的属性名称。例如，0 对应 'hex1'，1 对应 'round1'，以此类推。
    # model = torch.load('./checkpoint/reason_model_zero_1.0_4neg.pkl', map_location=torch.device(device))
    model = torch.load(r"F:\study\3_5programtest\8_bolt_change_v3\data_plastic\plastic_data_set_v4_Industrial_camera_6plastic\4_1080p_9PM_arealight1largellittle_lowst_f3.1_striplight4l_4s\shape_incloud_colour_data_aug_200transet\checkpoint\model_shape_0.98611_4neg.pkl",map_location=torch.device(device))
    #从指定路径加载模型。这里加载的模型是训练过程中表现最好的模型，它用于推理。
    for i in range(len(attribute_out_dic)): #遍历属性字典中的每个属性。
...
def infer_checkpoint(model,atribute_out): #推理的函数
    #一个字典，将属性名称映射到相应的螺栓类型。例如，'hex1' 对应 'out_hex_bolt'，'round1' 对应 'in_hex_bolt'，以此类推。
    attribute_out2bolt={'square1_abs':'absblack_black_and_square','square2_pared':'pared_red_and_square','square3_ppwhite':'ppwhite_white_and_square','irregularity1_pe':'pe_white_and_irregularity','irregularity2_ppblack':'ppblack_black_and_irregularity','irregularity3_pvc':'pvc_transwhite_and_irregularity','square4_pa':'pa_transwhite_and_square','square5_abswhite':'abswhite_white_and_square','irregularity4_pu':'pu_transwhite_and_irregularity'}
    attribute_out2index={'square1_abs':1,'square2_pared':1,'square3_ppwhite':1,'irregularity1_pe':0,'irregularity2_ppblack':0,'irregularity3_pvc':0,'square4_pa':1,'square5_abswhite':1,'irregularity4_pu':0} #这里对应着哪个位置的标签索引，其中，0位置对应标签0，1位置对应标签1
    # 一个字典，将属性名称映射到相应的索引。例如，'hex1' 对应 0，'round1' 对应 1，以此类推。
    op_list = [
            {'op': 'objects', 'param': ''},
            {'op':'filter_nearest_obj', 'param': ''},
    ...
    def __init__(self, file):
        ...
        with open(file) as f:
            ...
            for i, line in enumerate(lines): #使用enumerate遍历文件的每一行，获取行号i和行内容line。
                # if i == 0:
                #     continue
                (img_path, attribute_in) = line.split(' ')  # dataset triple 格式 #将每行内容按空格分割，得到图像路径img_path和属性信息attribute_in。
