...
DATA_INPUT = r"F:\study\3_5programtest\8_bolt_change_v3\data_plastic\plastic_data_set_v4_Industrial_camera_6plastic\4_1080p_9PM_arealight1largellittle_lowst_f3.1_striplight4l_4s\colour_data_aug_200transet/"  #用于生成训练集与测试集路径
...
def get_args_parser():
    ...
    parser.add_argument('--finetune', default=r"F:\study\3_5programtest\8_bolt_change_v3\model\mae_pretrain_vit_base.pth",
...
def train(model):
    ...
    train_set = TripleDataset(DATA_INPUT + 'attribute_four_colour_sixplastic_train.tsv')
    test_set = TripleDataset(DATA_INPUT + 'attribute_four_colour_sixplastic_test.tsv')  # use triple
    #用于训练和测试的数据集。使用 TripleDataset 类加载训练集和测试集。
    lr = 0.001
    epoch_num = 100
    loss_function = nn.MSELoss() # 超参数：损失函数
    ...
    for epoch in range(epoch_num): #迭代 epoch_num 次。外层迭代，表示训练过程的轮次。
        ...
        for i in index_list: #内层迭代，遍历索引列表，获取训练样本。此时i为打乱后第一个样本的原序号
            ...
            for i in range(4): #对于每个样本，执行两次迭代################################change(1/9)
                optimizer.zero_grad() #清零梯度
                # start_time = time.time()
                # print("start time begin")
                y_pred = model(op_lists[i], img,img_file_path, mode='train') #使用模型进行前向传播
                model.concept_matrix2zero() #模型的 concept_matrix 置零。
                

                loss = loss_function(y_pred, answers[i]) #计算损失并进行反向传播和参数更新。
        ...
        for i in range(test_set.len): #执行测试阶段
            ...
            for i in range(4): #对于每个样本，执行两次迭代。################################change(2/10)
        ...
        print('[INFO] ---train---')
        print('[INFO]---- train loss ----:', train_loss /( train_set.len*4))################################change(10/10)
        ...
        print('[INFO]---- test loss ----:', test_loss / (test_set.len*4))
        print('[INFO] ---eval---')
        print('[INFO]---- test acc ----:', acc / (test_set.len*4))
        ...
        # if F1 >= best_score:
        if round(acc / (test_set.len*4), 5) >= best_score:
            #如果测试准确率（acc）达到某个阈值（best_score），则保存当前模型。
            # best_score = round(F1, 2)
            best_score = round(acc / (test_set.len*4), 5)
            name_str = r'./data_plastic\plastic_data_set_v4_Industrial_camera_6plastic\4_1080p_9PM_arealight1largellittle_lowst_f3.1_striplight4l_4s\morelight_g2_colouraug200\checkpoint/model_aug_fourcolour_best_4neg.pkl'.replace('best', str(best_score))
...
def iii():
    ...
    attribute_out_dic={0:'black1_absblack',1:'white1_abswhite',2:'red1_pared',3:'white2_pe',4:'black2_ppblack',5:'white3_ppwhite',6:'transwhite1_pu',7:'transwhite2_pvc'} #一个字典，将索引映射到相应的属性名称。例如，0 对应 'hex1'，1 对应 'round1'，以此类推。
    # model = torch.load('./checkpoint/reason_model_zero_1.0_4neg.pkl', map_location=torch.device(device))
    model = torch.load(r"F:\study\3_5programtest\8_bolt_change_v3\data_plastic\plastic_data_set_v4_Industrial_camera_6plastic\4_1080p_9PM_arealight1largellittle_lowst_f3.1_striplight4l_4s\morelight_g2_colouraug200\checkpoint\model_aug_fourcolour_0.99861_4neg.pkl",map_location=torch.device(device))
...
def infer_checkpoint(model,atribute_out): #推理的函数
    ...
    attribute_out2bolt={'black1_absblack':'absblack_black_and_square','white1_abswhite':'abswhite_white_and_square','red1_pared':'pared_red_and_square','white2_pe':'pe_white_and_irregularity','black2_ppblack':'ppblack_black_and_irregularity','white3_ppwhite':'ppwhite_white_and_square','transwhite1_pu':'pu_transwhite_and_irregularity','transwhite2_pvc':'pvc_transwhite_and_irregularity'}
    attribute_out2index={'black1_absblack':2,'white1_abswhite':3,'red1_pared':1,'white2_pe':3,'black2_ppblack':2,'white3_ppwhite':3,'transwhite1_pu':0,'transwhite2_pvc':0} #这里对应着哪个位置的标签索引，其中，0位置对应标签0，1位置对应标签1
    # 一个字典，将属性名称映射到相应的索引。例如，'hex1' 对应 0，'round1' 对应 1，以此类推。
    op_list = [
            {'op': 'objects', 'param': ''},
            {'op':'filter_nearest_obj', 'param': ''},
            {'op':'obj_attibute', 'param': [0,attribute_out2index[atribute_out]]}
    ] #一个包含操作的列表，每个操作是一个字典，指定了操作的类型和参数
    img_list = list(range(200, 240)) #一个包含待推理的图像ID的列表。在这里，img_list 中只包含一个图像ID。
    # img_list = [1]
    print('[INFO]---------- 评分测试 ---------')
    tol_num=0
    val_num=0
...
class TripleDataset(Dataset): #用于加载训练和测试数据。
            ...
            for i in range(4): #构造测试集的操作列表，由于该属性有两个概念，每个概念要一个操作列表，由于神经网络输出两个概念的值，故每个操作列表的概念矩阵索引分别对应了各自concept的位置
                ...
                q={
                    ...
                    "op_list": [
                        ...
                        {"op": "obj_attibute", "param":[0,i]}],  # 通过filter判定该物体是否是triple中subject物体
    ...
    def __getitem__(self, index):
        ...
        for i in range(4): #循环遍历两个操作列表，并将它们添加到 op_lists 中。
            ####################################change(8/9)
            op_list = self.questions[index][i]['op_list']
            op_lists.append(op_list)
        answers=[] #初始化一个空列表，用于存储两个答案。
        for i in range(4): #循环遍历两个答案，并将它们添加到 answers 中。
            ####################################change(9/9)
...
if __name__ == "__main__":
    ...
    # Model Load
    model_colour = Reasoning(args) #初始模型
    # for name, param in model_colour.named_parameters(): #named_parameters()函数用于获取模块参数的标准方式，他使得可以方便地访问和操作神经网络模型中的参数。
    #     print(name, param.size(), type(param))
    # model_white = torch.load('./checkpoint/net_in_0.9862.pkl',map_location=torch.device(device)) #导入之前训练好的模型

    # train(model_colour) # 训练流程
